...
class MultiStepSRSolver(Solver):
    def __init__(self, graph, segment, timeout, verbose):
        super(MultiStepSRSolver, self).__init__(graph, timeout, verbose)
    ...
    def create_problem(self, tm):
        ...
        for u, v in self.G.edges:
            ...
            for t in range(tm.shape[0]):
                load = pl.lpSum(
                    x[flatten_index(i, j, k, self.num_node)] * tm[t, i, j] * edge_in_segment(self.segments, i, j, k, u,
                                                                                             v)
                    for i, j, k in
