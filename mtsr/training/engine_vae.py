...
from mtsr.compressive_sensing.utils import get_phi, get_psi
from mtsr.compressive_sensing.pursuit import sparse_coding
...
class TrainEngine_VAE:
    def __init__(self, data, model, args):
        ...
        self.save_path = os.path.join(self.args.model_folder, self.monitor.label)
        self.model = model
        self.model.set_save_path(self.save_path)
    ...
    def train(self):
        ...
        x_train_scaled = self.data['train/scaled']
        n_train_samples = x_train_scaled.shape[0]
        x_val_scaled = self.data['val/scaled']
        n_val_samples = x_val_scaled.shape[0]

        x_train_scaled = np.reshape(x_train_scaled,
                                    newshape=(n_train_samples, self.args.num_node, self.args.num_node))
        x_train_scaled = np.expand_dims(x_train_scaled, -1)

        x_val_scaled = np.reshape(x_val_scaled,
                                  newshape=(n_val_samples, self.args.num_node, self.args.num_node))
        x_val_scaled = np.expand_dims(x_val_scaled, -1)

        self.model.set_X_val(x_val_scaled)
        history = self.model.fit(x_train_scaled, epochs=self.num_epochs, batch_size=128,
                                 callbacks=[EarlyStoppingAtMinLoss()], )
        if not os.path.exists(self.save_path):
            os.makedirs(self.save_path, exist_ok=True)
        self.model.encoder.save(os.path.join(self.save_path, 'encoder'))
        self.model.decoder.save(os.path.join(self.save_path, 'decoder'))
