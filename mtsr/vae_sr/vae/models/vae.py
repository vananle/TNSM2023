...
class VAE(tf.keras.Model):
    ...
    def __init__(self, X_val, args, **kwargs):
        ...
        elif path.exists(os.path.join(args.log_dir, 'encoder')) and path.exists(os.path.join(args.log_dir, 'decoder')):
        ...
        if not load_saved_model:
            ...
            x = tf.keras.layers.Dense(args.num_node * args.num_node * 1, activation="relu")(x)
            decoder_outputs = tf.keras.layers.Reshape((args.num_node, args.num_node, 1))(x)
            ...
            self.encoder = tf.keras.models.load_model(os.path.join(args.log_dir, 'encoder'))
            self.decoder = tf.keras.models.load_model(os.path.join(args.log_dir, 'decoder'))
