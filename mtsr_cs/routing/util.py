...
def load_network_topology(dataset, datapath):
    ...
    for _, row in df.iterrows():
        ...
        G.add_edge(i, j, weight=row.weight,
                   capacity=row.bw,
                   delay=row.delay)
    print('Dataset: {} - nNodes: {} - nEdges: {}'.format(dataset, G.number_of_nodes(), G.number_of_edges()))
    return G
    ...
    if i != j:
        ...
        for k in range(N):
            try:
                ...
                print('--->>> (NO PATH) i: {}; j: {}; k: {} <<<---'.format(i, j, k))
                pass
        return path_edges
    else:
        return []
...
def get_segments(graph):
    n = graph.number_of_nodes()
    segments = {}
    segments_edges = Parallel(n_jobs=os.cpu_count())(delayed(get_paths)(graph, i, j)
...
def compute_path(graph, dataset, datapath):
    ...
    if os.path.exists(path):
        ...
        print('|--- Compute segments and save to {}'.format(path))
